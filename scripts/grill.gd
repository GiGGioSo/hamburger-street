...
@onready var slots := $HamburgerGrillSlots
...
func _exit_tree() -> void:
	...
	for hamburger in connected_hamburgers.duplicate():
		...
		var drag := hamburger.get_node_or_null("DraggableComponent") as DraggableComponent
...
func _process(delta: float) -> void:
	for slot in cooking_hamburgers.keys():
		...
		var hamburger := cooking_hamburgers.get(slot) as Hamburger
		if not is_instance_valid(hamburger):
			_stop_cooking(slot)
			continue
		...
		var elapsed := float(cooking_elapsed.get(slot, 0.0)) + delta
...
func _connect_grill_slots() -> void:
	for slot_container in slots.get_children():
		var slot := slot_container as SnapSlotComponent
		if slot == null:
...
func _on_slot_item_snapped(item: Node2D, slot: SnapSlotComponent) -> void:
	var hamburger := item as Hamburger
	if hamburger == null:
		return
